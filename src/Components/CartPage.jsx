  ...
  if (cartItems.length == 0) {
    return (
      <div className="mx-5 md:mx-32 mt-10 flex gap-2 items-center">
        <span className="text-lg font-title text-gray-700">
          Opps...., Looks like the cart is empty.
        </span>
        <button
          className="border-2 border-gray-400 px-1 rounded-md font-title"
          onClick={() => navigate(-1)}
        >
          Return To Shop
        </button>
          ...
          cartItems.map((item) => (
            ...
            >
              ...
              <div className="border-2 border-gray-300 w-16 px-4 py-2 flex items-center rounded-md">
                {!updateCartItem && (
                  <span className="text-left"> {item.orderQuantity} </span>
                )}
                {updateCartItem && (
                  <input
                    ...
                    max={item.stock}
                    onChange={(e) =>
                      updateOrderQuantity(parseInt(e.target.value), item.id)
                    }
                    type="number"
                    className="font-title font-medium focus:outline-none border-none w-10 text-left"
                  />
                )}
              </div>
